...
const Countdown = ({ setCountdownTitle }) => {
  const [countdownTime, setCountdownTime] = useState([]);

  const handleCalculatorCountdown = () => {
    const now = new Date();
    const Tet = new Date(2022, 0, 31);
    ...
    if (remTime > 0) {
      ...
      setCountdownTitle('Chúc mừng năm mới');
      setCountdownTime([
        { title: 'Ngày', value: 0 },
        { title: 'Giờ', value: 0 },
        { title: 'Phút', value: 0 },
...
export default function Firework() {
  const [countdownTitle, setCountdownTitle] = useState('Đếm ngược đến tết ');
    ...
    <div style={{ marginTop: 40 }}>
      ...
      <div className={styles.countdownWrapper}>
        <ul className={styles.countdownList}>
          <Countdown setCountdownTitle={setCountdownTitle} />
        </ul>
      </div>
